...
class MapsLauncher {
  ...
  static Uri createQueryUri(String query, {String? address}) {
    Uri uri;
    ...
    if (kIsWeb) {
      ...
      Map<String, dynamic>? queryParameters = {'q': query};
      uri = Uri(scheme: 'geo', host: '0,0', queryParameters: queryParameters);
    } else if (Platform.isIOS) {
      Map<String, dynamic>? queryParameters = {'q': query, 'address': address};
      queryParameters.removeWhere((key, value) => value ==null);
      uri = Uri.https('maps.apple.com', '/', queryParameters);
    } else {
      uri = Uri.https(
          'www.google.com', '/maps/search/', {'api': '1', 'query': query});
    }
  ...
  static Future<bool> launchQuery(String query, {String? address}) {
    return launchUrl(createQueryUri(query, address: address), mode: LaunchMode.externalApplication);
  }
