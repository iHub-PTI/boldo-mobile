...
void main() {
  test('Change appointment status from open to closed', () {
    // initial status
    final Appointment _appointment = Appointment(status: 'open');
  ...
  test('Change appointment status from locked to cancelled', () {
    // initial status
    final Appointment _appointment = Appointment(status: 'locked');
    ...
    expect(() => changeStatus.call(),
        throwsA(isA<InvalidAppointmentStatusChange>()));
  });
}
