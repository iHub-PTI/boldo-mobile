...
class OrganizationsSubscribedScreen extends StatefulWidget {
  ...
  @override
  _OrganizationsSubscribedScreenState createState() =>
      _OrganizationsSubscribedScreenState();
}

class _OrganizationsSubscribedScreenState
    extends State<OrganizationsSubscribedScreen> {
      ...
      providers: [
        BlocProvider<subscribed.OrganizationSubscribedBloc>(
          create: (BuildContext context) => GetIt.I
              .registerSingleton<subscribed.OrganizationSubscribedBloc>(
                  subscribed.OrganizationSubscribedBloc())
            ..add(subscribed.GetOrganizationsSubscribed(
                patientSelected: patientSelected)),
        ),
        BlocProvider<applied.OrganizationAppliedBloc>(
          create: (BuildContext context) => GetIt.I
              .registerSingleton<applied.OrganizationAppliedBloc>(
                  applied.OrganizationAppliedBloc())
        ...
        body: SafeArea(
          child: Container(
            child: MultiBlocListener(
              listeners: [
                BlocListener<subscribed.OrganizationSubscribedBloc,
                    subscribed.OrganizationSubscribedBlocState>(
                  listener: (context, state) {
                    if (state is subscribed.OrganizationsSubscribedObtained) {
                      ...
                      emitSnackBar(
                          context: context,
                          text:
                              "Suscripcion a ${_organizationsSubscribed.firstWhere((element) => element.id == state.id).name} eliminada",
                          status: ActionStatus.Success);

                      _organizationsSubscribed
                          .removeWhere((element) => element.id == state.id);

                      // notify the home to remove an organization
                      BlocProvider.of<home_organization.HomeOrganizationBloc>(
                              context)
                          .add(home_organization.GetOrganizationsSubscribed());
                ...
                BlocListener<applied.OrganizationAppliedBloc,
                  ...
                  listener: (context, state) {
                    if (state is applied.OrganizationsObtained) {
                      ...
                      emitSnackBar(
                          context: context,
                          text:
                              "Solicitud a ${_organizationsPostulated.firstWhere((element) => element.id == state.id).organizationName} cancelada",
                          status: ActionStatus.Success);
                      _organizationsPostulated
                          .removeWhere((element) => element.id == state.id);
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                              ...
                              child: Builder(builder: (context) {
                                return FamilySelector(
                                  ...
                                  actionCallback: (_patientSelected) {
                                    ...
                                    BlocProvider.of<
                                                subscribed
                                                .OrganizationSubscribedBloc>(
                                            context)
                                        .add(subscribed
                                            .GetOrganizationsSubscribed(
                                                patientSelected:
                                                    patientSelected));
                                    BlocProvider.of<
                                            applied
                                            .OrganizationAppliedBloc>(context)
                                        .add(applied.GetOrganizationsPostulated(
                                            patientSelected: patientSelected));
                                  },
                                );
                    ...
                    BlocBuilder<subscribed.OrganizationSubscribedBloc,
                            subscribed.OrganizationSubscribedBlocState>(
                      ...
                      if (state is subscribed.Failed) {
                        child = DataFetchErrorWidget(
                            retryCallback: () => BlocProvider.of<
                                    subscribed
                                    .OrganizationSubscribedBloc>(context)
                                .add(subscribed.GetOrganizationsSubscribed(
                                    patientSelected: patientSelected)));
                              ...
                              child: Container(
                                  ...
                                  child: Column(children: [
                                          ...
                                          InkWell(
                                            onTap: () async {
                                              ...
                                              if (result == true) {
                                                ...
                                                BlocProvider.of<
                                                            applied
                                                            .OrganizationAppliedBloc>(
                                                        context)
                                                    .add(applied
                                                        .GetOrganizationsPostulated(
                                                            patientSelected:
                                                                patientSelected));
                                              }
                                            },
                                    ...
                                    ListView.builder(
                                      ...
                                      shrinkWrap: true,
                                      itemCount:
                                          _organizationsSubscribed.length,
                    ...
                    BlocBuilder<applied.OrganizationAppliedBloc,
                            applied.OrganizationAppliedBlocState>(
                        builder: (context, state) {
                      Widget child;
                      if (state is applied.Failed) {
                        child = DataFetchErrorWidget(
                            retryCallback: () => BlocProvider.of<
                                    applied.OrganizationAppliedBloc>(context)
                                .add(applied.GetOrganizationsPostulated(
                                    patientSelected: patientSelected)));
                              ...
                              child: Container(
                                  ...
                                  child: Column(children: [
                                    ...
                                    ListView.builder(
                                      ...
                                      shrinkWrap: true,
                                      itemCount:
                                          _organizationsPostulated.length,
  ...
  Widget emptyView(BuildContext context) {
    return Container(
      decoration: BoxDecoration(color: ConstantsV2.grayLightest, boxShadow: [
        shadowRegular,
      ]),
      padding: const EdgeInsets.all(16),
      child: Row(
          ...
          InkWell(
            onTap: () async {
              dynamic result = await Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        OrganizationsScreen(patientSelected: patientSelected)),
              );
              if (result == true) {
                BlocProvider.of<subscribed.OrganizationSubscribedBloc>(context)
                    .add(subscribed.GetOrganizationsSubscribed(
                        patientSelected: patientSelected));
                BlocProvider.of<applied.OrganizationAppliedBloc>(context).add(
  ...
  Widget _cardFunder(Organization organization) {
    return OrganizationSubscribedCard(
  ...
  Widget _cardFunderPostulated(OrganizationRequest organization) {
    return OrganizationPostulationCard(
        organization: organization, patientSelected: patientSelected);
  }
...
class OrganizationPostulationCard extends StatelessWidget {
  ...
  Widget cancelSubscriptionOption(
      OrganizationRequest organization, BuildContext context) {
    return InkWell(
      onTap: () async {
        String? action = await cancelApplication(context, organization);
        if (action == 'cancel') {
          BlocProvider.of<applied.OrganizationAppliedBloc>(context).add(
              applied.UnPostulated(
                  organization: organization,
                  patientSelected: patientSelected));
        }
      },
  ...
  Future<String?> cancelApplication(
      BuildContext context, OrganizationRequest organization) {
    return showDialog<String>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: const Text('Cancelar solicitud pendiente'),
        content: Text(
            '¿Desea cancelar la solicitud a ${organization.organizationName}?'),
        actions: <Widget>[
          TextButton(
...
class OrganizationSubscribedCard extends StatelessWidget {
  OrganizationSubscribedCard(
      {required this.organization, required this.patientSelected});
...
class FamilySelectorState extends State<FamilySelector> {
  ...
  void initState() {
    ...
    BlocProvider.of<family_bloc.FamilyBloc>(context)
        .add(family_bloc.GetFamilyList());
  }
                  ...
                  child: Row(
                    children: [
                      ...
                      ..._families
                          ...
                          .entries
                          .map((e) =>
                              _buildPictureRoundedFamily(context, e.key + 1))
                          .toList(),
                    ],
                  ),
                ),
      ...
      child: GestureDetector(
        onTap: () => {
          setState(() {
            widget.patientSelected =
                index == 0 ? _patient : families[index - 1];
          }),
        ...
        child: Column(
          children: [
            ...
            Text(
              index == 0
                  ? _patient.givenName?.split(' ').first ?? ''
                  : families[index - 1].givenName?.split(' ').first ?? '',
  ...
  Widget _profileFamily(int index, String type) {
    ...
    bool disable = index == 0
        ? widget.patientSelected.id == prefs.getString("userId")
            ? false
            : true
        : widget.patientSelected.id == families[index - 1].id
            ? false
            : true;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 4),
