...
import 'package:boldo/blocs/lastAppointment_bloc/lastAppointmentBloc.dart'
    as last_appointment_bloc;
...
class DoctorProfileScreen extends StatefulWidget {
  ...
  const DoctorProfileScreen(
      {Key? key, required this.doctor, this.showAvailability = false})
      : super(key: key);
  final Doctor doctor;
        ...
        child: MultiBlocProvider(
          providers: [
            BlocProvider<last_appointment_bloc.LastAppointmentBloc>(
              create: (BuildContext context) =>
                  last_appointment_bloc.LastAppointmentBloc()
                    ..add(
                      last_appointment_bloc.GetLastAppointment(
                          doctor: widget.doctor),
                    ),
            ),
            BlocProvider<doctor_bloc.DoctorBloc>(
              create: (BuildContext context) => doctor_bloc.DoctorBloc()
                ..add(
                  doctor_bloc.GetAvailability(
                    ...
                    organizations: Provider.of<DoctorFilterProvider>(context,
                                listen: false)
          ...
          child: MultiBlocListener(
            listeners: [
              ...
              BlocListener<last_appointment_bloc.LastAppointmentBloc,
                      last_appointment_bloc.LastAppointmentState>(
                  listener: (context, state) {
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                              ...
                              children: [
                                BlocBuilder<
                                        last_appointment_bloc.LastAppointmentBloc,
                                        last_appointment_bloc
                                        .LastAppointmentState>(
                                    builder: (context, state) {
                                  return AnimatedOpacity(
                                    duration:
                                        const Duration(milliseconds: 1000),
                                    opacity: state is last_appointment_bloc
                                                .LastAppointmentLoadedState &&
                                            lastAppointment != null
                                        ? 1.0
                                        : 0.0, // 1 is to get visible
                                    child: Visibility(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ImageViewTypeForm(
                                                ...
                                                width: 44,
                                                border: true,
                                                borderColor: ConstantsV2
                                                    .secondaryRegular,
                                                gender: lastAppointment
                                                    ?.patient?.gender,
                                                url: lastAppointment
                                                    ?.patient?.photoUrl,
                                              ),
                                ...
                                if (widget.showAvailability)
                                  BlocBuilder<doctor_bloc.DoctorBloc,
                                          doctor_bloc.DoctorState>(
                                      builder: (context, state) {
                                    if (state
                                        is doctor_bloc.AvailabilitiesObtained) {
                                      if (organizationsWithAvailabilites
                                          .isNotEmpty)
                                        return ClipRect(
                                          child: Container(
                                            ...
                                            child: BackdropFilter(
                                                ...
                                                filter: ImageFilter.blur(
                                                    sigmaX: 5, sigmaY: 5),
                                                child: organizationsWithAvailabilites
                                                        .isNotEmpty
                  ...
                  children: [
                    if (organizationsWithAvailabilites[index]
                      ...
                      if (DateTime(
                        ...
                        Expanded(
                            child: Text(
                          "Disponible el ${DateFormat('dd/MM').format(DateTime.parse(organizationsWithAvailabilites[index].nextAvailability?.availability ?? DateTime.now().toString()))}"
                          " - $organizationName",
                          style: boldoScreenSubtitleTextStyle.copyWith(
                              color: ConstantsV2.grayLightest),
                        ))
        ...
        Container(
          ...
          child: Wrap(
            ...
            children: [
              ...
              if (organizationsWithAvailabilites[index].availabilities.length >
                  1)
                Container(
                  padding:
        ...
        child: GestureDetector(
          onTapDown: (TapDownDetails details) async {
            DateTime parsedAvailability = DateTime.parse(
                organizationsWithAvailabilites[indexOrganization]
                        ...
                        ?.availability ??
                    DateTime.now().toString());
            if (organizationsWithAvailabilites[indexOrganization]
                    .availabilities[index]
                    ?.appointmentType! ==
                'AV') {
              print(details.globalPosition);
              final chooseOption = await _showPopupMenu(details.globalPosition);
          ...
          child: Container(
            ...
            decoration: ShapeDecoration(
                ...
                shape: RoundedRectangleBorder(
                  side: BorderSide(
                      color: ConstantsV2.grayLightAndClear, width: 1),
                  borderRadius: BorderRadius.circular(100),
                ),
                shadows: [
                  shadowHourAvailable,
  ...
  Future<String?>? _showPopupMenu(Offset offset) async {
    ...
    return await showMenu(
      ...
      position: RelativeRect.fromLTRB(
          left, top, MediaQuery.of(context).size.width - left, 0),
